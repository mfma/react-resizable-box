...
type Callback = (event: MouseEvent | TouchEvent,
                 direction: Direction,
                 refToElement: HTMLElement,
                 delta: NumberSize,) => void;

type ResizeStartCallBack = (e: SyntheticMouseEvent | SyntheticTouchEvent,
                            dir: Direction,
                            refToElement: HTMLElement,) => void;
...
export default class Resizable extends Component {
	...
	onMouseMove(event: MouseEvent | TouchEvent) {
		...
		const {lockAspectRatio, minWidth, minHeight,parentScale} = this.props;
		let {maxWidth, maxHeight} = this.props;
		const ratio = original.height / original.width;
		let newWidth = original.width;
		let newHeight = original.height;
		if (/right/i.test(direction)) {
			newWidth = original.width + (clientX - original.x)/parentScale;
		...
		if (/bottom/i.test(direction)) {
			newHeight = original.height + (clientY - original.y)/parentScale;
